...
struct rt_event event_per,event_done,event_dist;
...
int msgq_init(void)
{
		...
		rt_mb_init(&s_tar_mb[0],"lf_tar",&lf_s_tar[0],4,RT_IPC_FLAG_FIFO);
		rt_mb_init(&s_tar_mb[1],"lf_tar",&ri_s_tar[0],4,RT_IPC_FLAG_FIFO);
	
	  rt_mb_init(&ele_mb[0],"lf_ele",lf_ele,4,RT_IPC_FLAG_FIFO);
		rt_mb_init(&ele_mb[1],"ri_ele",ri_ele,4,RT_IPC_FLAG_FIFO);
	
		rt_mb_init(&total_mb[0],"ang_left",&total_left[0],4,RT_IPC_FLAG_FIFO);
		rt_mb_init(&total_mb[1],"ang_right",&total_right[0],4,RT_IPC_FLAG_FIFO);
	
		rt_mb_init(&dis_tar_mb[0],"dis_left",&dis_tar_left[0],4,RT_IPC_FLAG_FIFO);
		rt_mb_init(&dis_tar_mb[1],"dis_right",&dis_tar_right[0],4,RT_IPC_FLAG_FIFO);
